...
class GestionStock:
    ...
    def login_screen(self):
        ...
        ctk.CTkLabel(login_window, text="Rol").pack(pady=5)
        rol_combobox = ctk.CTkComboBox(login_window, values=["admin", "normal"], state='readonly')
        rol_combobox.set("normal")
        ...
        def login():
            ...
            user = self.autenticar_usuario(usuario, contrasena, rol)
            if user:
        ...
        ctk.CTkButton(login_window, text="Crear Usuario", command=crear_usuario).pack(pady=10)

    def autenticar_usuario(self, usuario, contrasena, rol):
        hashed_contrasena = Usuario._hash_contrasena(contrasena)
        for user in self.usuarios:
            if user.usuario == usuario and user.contrasena == hashed_contrasena and user.rol == rol:
                return user
        return None
